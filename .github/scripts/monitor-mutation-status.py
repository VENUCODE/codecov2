...
def run_cr_report(db_file):
    ...
    try:
        ...
        print(f"cr-report not found. Make sure cosmic-ray is installed.", file=os.sys.stderr)
        return ""
...
def extract_status_with_grep(output):
    ...
    for line in lines:
        ...
        if "surviving mutants:" in line.lower():
            ...
            parts = line.split()
            for i, part in enumerate(parts):
                if part.lower() == "surviving" and i + 1 < len(parts) and parts[i+1].lower().startswith("mutants"):
                    try:
